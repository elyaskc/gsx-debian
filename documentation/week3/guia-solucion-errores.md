...
De manera manual podemos ver los procesos que más CPU o Memoria esten usando con: 

`ps aux --sort=-%cpu` o `ps aux --sort=-%mem`
...
Después, una vez analizado las métricas, debemos tomar acciones. Si el proceso funciona pero consume demasiado, podemos probar a bajarle prioridad con `renice`.
Y si por otro lado el proceso no responde, podemos hacerle kill, primero con -SIGTERM y si sigue vivo, usar -SIGKILL.
